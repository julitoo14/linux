...
<template>
  <div class="fixed top-0 left-0 w-full h-8 bg-slate-900/60 backdrop-blur-md text-white flex items-center justify-between px-4 z-9999 border-b border-white/5 text-sm select-none shadow-sm">
    ...
    <div class="relative">
      ...
      >
        <button @click="handleMenuAction('about')" class="text-left px-4 py-2 hover:bg-blue-600 hover:text-white transition-colors">
          Sobre este OS
        </button>
        <button @click="handleMenuAction('github')" class="text-left px-4 py-2 hover:bg-blue-600 hover:text-white transition-colors">
          Ver Código Fuente
        </button>
    ...
    <div class="absolute left-1/2 -translate-x-1/2 font-medium opacity-90 hover:opacity-100 cursor-default">
      {{ date }} &nbsp; {{ time }}
    </div>

    <div class="flex items-center gap-3">

      <div class="flex items-center gap-2 hover:bg-white/10 px-2 py-1 rounded cursor-pointer transition-colors">
        <span class="font-semibold hidden sm:block">Julián García</span>
        <div class="w-5 h-5 bg-linear-to-tr from-green-400 to-blue-500 rounded-full border border-white/20"></div>
      </div>
    </div>
